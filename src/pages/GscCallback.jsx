...
export default function GscCallback() {
  const [status, setStatus] = useState('Connecting to Google Search Console...')
  ...
  useEffect(() => {
    ...
    const exchange = async () => {
      try {
        setStatus('Fetching your Search Console data...')
        const res = await fetch('https://ezltbarrkvlfijbkwwam.supabase.co/functions/v1/connect-gsc?action=callback', {
          method: 'POST',
